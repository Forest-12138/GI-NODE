...
SPF = 0.25
M = max(1, int(H * W * SPF))
LATENT_DIM = 128  # 论文 2D 流实验中 Latent Dim 设为 64
NOISE_LEVEL = 0.02
...
# 1. 数据加载（圆周平移 MNIST）
# ==========================================
print("1. Loading Circular Translation MNIST...")


def apply_circular_motion(images):
    images_padded = F.pad(images, (18, 18, 18, 18))
    seqs = []
    R = 14.0
    for t in range(T):
        angle = 2 * np.pi * t / T
        dx, dy = R * np.cos(angle), R * np.sin(angle)
        frame = TF.affine(images_padded, angle=0.0,
                          translate=[float(dx), float(dy)],
                          scale=1.0, shear=0.0)
        seqs.append(frame.squeeze(1))
    return torch.stack(seqs, dim=1).to(device)


full_dataset = MNIST(root='./data', train=True, download=True, transform=transforms.ToTensor())
test_dataset = MNIST(root='./data', train=False, download=True, transform=transforms.ToTensor())

train_images, _ = next(iter(torch.utils.data.DataLoader(full_dataset, batch_size=TRAIN_BATCH, shuffle=True)))
test_images, _ = next(iter(torch.utils.data.DataLoader(test_dataset, batch_size=1, shuffle=True)))

x_train_gt = apply_circular_motion(train_images)  # [B, T, H, W]
x_test_gt = apply_circular_motion(test_images)[0]  # [T, H, W]
...
for epoch in range(STATIC_EPOCHS):
    ...
    with torch.amp.autocast(device_type=_AMP, enabled=torch.cuda.is_available()):
        ...
        loss = criterion_bce(logits, x_mb)
...
with torch.no_grad():
    ...
    test_recon = torch.sigmoid(decoder(test_z.unsqueeze(1)).squeeze(1))
...
# ---------------------------------------------------------
# Phase 4.2: 动态 ODE 联合训练 (冻结 Decoder)
# ---------------------------------------------------------
print("\n   [Phase 4.2] Dynamic ODE Training (Frozen Decoder Manifold)...")

# 【核心操作】：彻底冻结 Decoder，锁定锐利的形状空间
decoder.eval()
for p in decoder.parameters():
    p.requires_grad = False

# 仅优化 Encoder 和 ODE
# Encoder 需要微调其映射方式以配合 ODE，ODE 则学习流形上的动力学
opt_dyn = optim.Adam(list(encoder.parameters()) + list(ode_func.parameters()), lr=5e-4, weight_decay=1e-5)
...
for epoch in range(DYN_EPOCHS):
    ...
    with torch.amp.autocast(device_type=_AMP, enabled=torch.cuda.is_available()):
        ...
        # 3. 通过冻结的 Decoder 计算图像损失
        logits_pred = decoder(z_pred)
        xr_pred = torch.sigmoid(logits_pred)

        # 目标一：ODE 预测的轨迹，解码后必须和真实视频一样
        loss_pred_img = criterion_bce(logits_pred, x_mb) + criterion_mse(xr_pred, x_mb)

        # 目标二：ODE 预测的潜变量，必须靠近 Encoder 给出的真实潜变量
        loss_latent = criterion_mse(z_pred, z_seq)

        # 目标三：Encoder 自己输出的特征解码后依然是对的 (维持 Encoder 不跑偏)
        with torch.no_grad():
            # 这一步不需要梯度，只用来做参考
            logits_ae = decoder(z_seq)

        # 因为 Decoder 是冻结的，所以必须依靠 BCE 强力惩罚偏差
        loss = loss_pred_img + 0.5 * loss_latent

    scaler.scale(loss).backward()
    scaler.unscale_(opt_dyn)
    torch.nn.utils.clip_grad_norm_(list(encoder.parameters()) + list(ode_func.parameters()), 1.0)
    scaler.step(opt_dyn)
    scaler.update()
    sched_dyn.step()
...
with torch.no_grad():
    ...
    for i in range(0, 256, MINI_BATCH):
        ...
        for j in range(z_cand.size(0)):
            x_est = torch.sigmoid(decoder(z_cand[j:j + 1])).squeeze(0)  # [T, H, W]
...
for epoch in range(200):
    ...
    x_est = torch.sigmoid(decoder(Z_ode.unsqueeze(0))).squeeze(0)
    y_est = torch.einsum('tmhw,thw->tm', A, x_est) / SCALE_FACTOR
...
for epoch in range(300):
    ...
    # 测量保真度项
    x_est = torch.sigmoid(decoder(Z_opt.unsqueeze(0))).squeeze(0)
    y_est = torch.einsum('tmhw,thw->tm', A, x_est) / SCALE_FACTOR
    loss_meas = nn.MSELoss()(y_est, y_measured)
...
with torch.no_grad():
    ...
    x_recon = torch.sigmoid(decoder(Z_final.unsqueeze(0))).squeeze(0).cpu().numpy()
# ==========================================
# 6. 可视化
# ==========================================
